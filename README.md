...
- **[build.component.pkg.sh](build.component.pkg.sh)**: Builds & signs a macOS component .pkg from a payload folder via `pkgbuild`, with optional `--scripts` and `--component-plist` payloads. Runs standalone, or is called by build.distribution.pkg.sh (same directory) for that script's component-pkg step.
- **[build.distribution.pkg.sh](build.distribution.pkg.sh)**: Builds a signed, optionally notarized (and optionally .dmg-wrapped) macOS distribution installer from a payload folder via build.component.pkg.sh (component pkg) + `productbuild` (distribution pkg), with optional `--scripts`, `--resources`, and `--component-plist` payloads. Requires a Developer ID Application & Installer certificate in the keychain, and build.component.pkg.sh present alongside it.
...
- **[lobotomy.sh](lobotomy.sh)**: Wipes Claude Code's local state: top-level caches/logs/scratch dirs, then per-project session data under `~/.claude/projects`, preserving each project's `memory` folder. An editable array at the top excludes specific project directories entirely.
- **[macOS.block.app.sh](macOS.block.app.sh)**: Installs a LaunchDaemon that kills a named app on launch and shows a blocking dialog; configured for Migration Assistant by default. Re-running is safe (unloads any existing daemon first); `--remove` uninstalls it. Must be run as root.
