...
- **[build.pkg.sh](build.pkg.sh)**: Builds & signs a macOS .pkg via `pkgbuild`/`productbuild`; mode is selected by a required `--component`/`-C` or `--distribution`/`-D` flag. Requires a Developer ID Application & Installer certificate in the keychain.`--component` builds a standalone component .pkg; `--distribution` builds a signed, optionally notarized (and optionally .dmg-wrapped) distribution installer. Both modes take optional `--scripts` and `--component-plist` payloads; `--distribution` also takes an optional `--resources` payload.
